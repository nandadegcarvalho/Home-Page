...
export default function ListaPage() {
    ...
    const [filmeNovo,setFilmeNovo] = useState ("")
    const handleChange = (event) => {
        setFilmeNovo(event.target.value);
    };
    const adicionaFilme=() => {
        // setFilmes(prev=>([...prev,filmeNovo]))
        setFilmes([...filmes,filmeNovo])
        setFilmeNovo("")
    }
    return (
        <div>
            <h1>Lista de Filmes da Marvel</h1>
            <input type="text" placeholder = "Digite um novo filme" onChange={handleChange} value={filmeNovo}/>
